...
--function
--1a
create function ca(@msp nchar(10))
returns nvarchar(20)
as
begin
declare @tenhangsx nvarchar(20)
...
--1b
create function cb(@x money, @y money, @z nchar(10))
returns int
as 
begin
declare @dem int;
set @dem= (select count(masp)
				...
				where gia between @x and @y and tenhang=@z
				group by tenhang)
return @dem
end

select dbo.cb(7000000,19000000,'OPPO')

--b2
--ca
create function cah(@x money, @y nvarchar(20))
returns @bang table(
					masp nchar(10),
					tensp nvarchar(20),
					sl int,
...
begin 
insert into @bang
select masp, tensp, sl, mau, gia
from sanpham join hangsx
on sanpham.mahangsx= hangsx.mahangsx
where gia>=@x and tenhang=@y
return 
end

select *from cah(3000000, 'SamSung')
