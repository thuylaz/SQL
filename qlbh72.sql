...
--function
--cau a
create function caua(@msp nchar(10))
returns nvarchar(20) 
as
begin
declare @tenhangsx nvarchar(20)
set @tenhangsx= (select tenhang from hangsx join sanpham
					on hangsx.mahangsx= sanpham.mahangsx
					where masp=@msp)
return @tenhangsx
end

select dbo.caua('SP05')
...
begin
	...
	select @tongtien= sum(sln*dongian)
...
select dbo.caub(2019, 2020)

--cau c
create function cauc(@tsp nvarchar(20), @nam int)
returns int
as
begin
...
--b2
--cau a
create function ca(@tenhangsx nvarchar(20))
returns @bang table(
			masp nchar(10),
			tensp nvarchar(20),
			sl int,
...
begin
	insert into @bang
		select masp, tensp, sl, mau, gia, donvitinh, mota
		from sanpham join hangsx
		on sanpham.mahangsx= hangsx.mahangsx
	where tenhang= @tenhangsx
return
end

select *from ca('Samsung')

--cau b
create function cb(@x date, @y date)
returns @bang table(
				masp nchar(10),
				tensp nvarchar(20),
				tenhang nvarchar(20),
...
begin
	insert into @bang
		select masp, tensp, tenhang, ngaynhap, sln, dongian
